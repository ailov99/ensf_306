...
function Base.getproperty(op::AbstractOp, f::Symbol)
    ...
    #elseif f == :fn && op.val === nothing
    #    #return Variable(Nothing, Nothing, Nothing)
    #    #return Nothing
    #    return nothing
    #    #return Variable(nothing, op)
    else
...
mutable struct Tape{C}
    ...
    ops::MutableLinkedList{<:AbstractOp}
    # result variable
    result::Variable
...
Tape(c::C) where C = Tape(MutableLinkedList{AbstractOp}(), Variable(0), nothing, Dict(), c)
...
Base.isempty(tape::Tape) = isempty(tape.ops)
...
function Base.push!(tape::Tape, op::AbstractOp)
    @timeit tmr "tape_push" begin
    new_id = length(tape) + 1
    op.id = new_id
    op.tape = tape

    if op isa Constant && op.val === nothing
        println("___CONST_WUTH_NOTHING__")
    end
...
function Base.insert!(tape::Tape, idx::Integer, node::DataStructures.ListNode{AbstractOp})
    return insert!(tape, idx, node.data)
end
...
function Base.insert!(tape::Tape, idx::Integer, ops::AbstractOp...)
    @timeit tmr "tape_insert" begin
        num_new_ops = length(ops)

        if num_new_ops != 0
            println("Insert_Called: at ", idx, " elements ", num_new_ops)
        end

        if num_new_ops != 0
            try
                insertData!(tape.ops, idx, ops)

                node_to_update = getNodeAt(tape.ops, idx) # init = first new node
                for i = idx:length(tape.ops)
                    node_to_update.data.id = i
                    node_to_update = node_to_update.next
                end
            catch
                println("INSERT_EXCEPT_", num_new_ops, " ", length(tape.ops))
            end
        end
    end
...
function Base.deleteat!(tape::Tape, idx::Integer; rebind_to = nothing)
    @timeit tmr "tape_deleteat" begin
        # delete and rebind
        delete!(tape.ops, idx)
