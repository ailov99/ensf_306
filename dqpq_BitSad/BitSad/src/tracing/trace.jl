...
function transform!(f, fctx, tape::Ghost.Tape)
    ...
    local current_node_data, rebind_to
    
    #itr = iterate(tape)
    #idx = 2
    
    current_node_data, next_node = iterate(tape)
    next_node_i = (length(tape) > 0) ? 2 : 0

    #while !isnothing(itr)
    while 0 < next_node_i < length(tape)
        #entry, idx_node = itr

        if current_node_data isa Ghost.Call
            new_entries, rebind_to = f(tape.c, current_node_data)
            if isempty(new_entries)
                deleteat!(tape, next_node_i - 1; rebind_to = rebind_to)
            else
                replace!(tape, next_node_i - 1 => new_entries; rebind_to = rebind_to)
                vars = [tape[Ghost.Variable(next_node_i - 1 + i)] for i in 0:(length(new_entries) - 1)]
                fctx(tape.c, vars)
            end

            next_node_i += length(new_entries) - 1
            for i = 1:(length(new_entries) - 1)
                next_node = next_node.next
            end
        elseif current_node_data isa Ghost.Input
            new_entries, rebind_to = f(tape.c, current_node_data)
            isempty(new_entries) && error("Cannot delete Ghost.Input")
            new_vars = insert!(tape, next_node_i, new_entries[2:end]...)
            vars = Ghost.AbstractOp[current_node_data]
