...
function IRTracer(; ctx=Dict(), is_primitive=is_chainrules_primitive)
    @timeit tmr "irtracer_tape_ctruction" tape = Tape(ctx)
    @timeit tmr "irtracer_irtr_ctor_block" begin
...
@dynamo function (t::IRTracer)(fargs...)
    @timeit tmr "ir_ctor" ir = IR(fargs...)
    ir === nothing && return   # intrinsic functions
    @timeit tmr "dynamo_expand" expand!(ir)
    @timeit tmr "dynamo_rewrite_special" rewrite_special_cases!(ir)
    @timeit tmr "dynamo_loop" begin
    for (v, st) in ir
        ex = st.expr
        # note the difference:
    ...
    @timeit tmr "dynamo_trace_branches" trace_branches!(ir)
    @timeit tmr "dynamo_trace_loops" trace_loops!(ir)
    return ir
end
...
function trace(f, args...; is_primitive=is_primitive, primitives=nothing, ctx=Dict())
    ...
    @timeit tmr "ghost_irtracer_ctor" t = IRTracer(; ctx=ctx, is_primitive=is_primitive)
    @timeit tmr "ghost_inputs_xform" arg_vars = inputs!(t.tape, f, args...)
    @timeit tmr "ghost_frame_ctor" frame = Frame(Dict(i => a for (i, a) in enumerate(arg_vars)), V(0), (f, args...))
    @timeit tmr "ghost_push_frame" push!(t.frames, frame)
    println("____GHOST_T_CTOR______")
    @timeit tmr "ghost_t_ctor" val = t(f, args...)
